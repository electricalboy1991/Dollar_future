...
atr_mean = atr_data['14-day ATR'].mean()
atr_std = atr_data['14-day ATR'].std()
# 수익 계산 함수
    ...
    for index, row in target_df.iterrows():
        ...
        if price > GC +n/2  or (price <= GC+n/2  and short_positions):
            ...
            if not short_positions and short_liquid_flag==0:
                ...
                short_positions.append({'short_target_price': target_price, 'datetime': datetime, 'price': price, 'contracts': (contracts+(atr_value - atr_mean) / atr_std)})
                total_num_cont = (contracts+(atr_value - atr_mean) / atr_std) + total_num_cont
                ...
                if 18 <= datetime.hour <= 23:
                    ...
                    tax_now =  ((price - tax_close_price) * (contracts+(atr_value - atr_mean) / atr_std) * tax_rate)
                elif 0 <= datetime.hour <= 4:
                    one_day_before = datetime - pd.Timedelta(days=1)
                    target_tax_time = pd.Timestamp(year=one_day_before.year, month=one_day_before.month, day=one_day_before.day, hour=15, minute=30)
                    if target_tax_time in target_df['datetime'].values:
                    ...
                    if short_positions[-1]['price'] + n < price and price >GC+n/2  :
                        ...
                        if (datetime.hour == 9 and datetime.minute == 0) or (datetime.hour == 18 and datetime.minute == 0):
                            ...
                            if short_positions[-1]['price'] + n < price_open:
                                ...
                                short_positions.append({'short_target_price': target_price, 'datetime': datetime, 'price': price_open, 'contracts': (contracts+(atr_value - atr_mean) / atr_std)})
                            else:
                                target_price = round((short_positions[-1]['price'] + n) * (1 - a), 4)
                                short_positions.append({'short_target_price': target_price, 'datetime': datetime, 'price': (short_positions[-1]['price'] + n), 'contracts': (contracts+(atr_value - atr_mean) / atr_std)})
                        ...
                        if 18 <= datetime.hour <= 23:
                            ...
                            if target_tax_time in target_df['datetime'].values:
                                ...
                                closest_time = target_df[target_df['datetime'] < target_tax_time]['datetime'].max()
                                tax_close_price = target_df[target_df['datetime'] == closest_time]['close'].iloc[0]
                            tax_now = (((short_positions[-1]['price']) - tax_close_price) * (contracts+(atr_value - atr_mean) / atr_std) * tax_rate)
                        elif 0 <= datetime.hour <= 4:
                            one_day_before = datetime - pd.Timedelta(days=1)
                            target_tax_time = pd.Timestamp(year=one_day_before.year, month=one_day_before.month, day=one_day_before.day, hour=15, minute=30)
                            if target_tax_time in target_df['datetime'].values:
        ...
        if price < GC-buffer-n/2  or (price >= GC-buffer-n/2  and long_positions):
            ...
            if not long_positions and long_liquid_flag==0:
                ...
                target_price = round(price * (1 + a), 4)
                long_positions.append({'long_target_price': target_price,'datetime': datetime,'price' : price, 'contracts': (contracts+(atr_value - atr_mean) / atr_std)})
                total_num_cont = (contracts+(atr_value - atr_mean) / atr_std) + total_num_cont
                    ...
                    if long_positions[-1]['price'] - n > price and price < GC-buffer-n/2 :
                        ...
                        if (datetime.hour == 9 and datetime.minute == 0) or (datetime.hour == 18 and datetime.minute == 0):
                            ...
                            if long_positions[-1]['price'] - n > price_open:
                                target_price = round(price_open * (1 + a), 4)
                                long_positions.append({'long_target_price': target_price, 'datetime': datetime, 'price': price_open, 'contracts': (contracts+(atr_value - atr_mean) / atr_std)})
                            # close 가격은 기준 그리드를 넘어서서 포지션을 잡아야하는데, open은 만족 x -> 기존 그리드 대로 포지션 잡기
                            else:
                                target_price = round((long_positions[-1]['price'] - n) * (1 + a), 4)
                                long_positions.append({'long_target_price': target_price, 'datetime': datetime, 'price': (long_positions[-1]['price'] - n), 'contracts': (contracts+(atr_value - atr_mean) / atr_std)})
                        else:
                        ...
                        if 18 <= datetime.hour <= 23:
                            ...
                            if target_tax_time in target_df['datetime'].values:
                                ...
                                closest_time = target_df[target_df['datetime'] < target_tax_time]['datetime'].max()
                                tax_close_price = target_df[target_df['datetime'] == closest_time]['close'].iloc[0]
                            tax_now =  -(((long_positions[-1]['price']) - tax_close_price) * (contracts+(atr_value - atr_mean) / atr_std) * tax_rate)
                        elif 0 <= datetime.hour <= 4:
                            one_day_before = datetime - pd.Timedelta(days=1)
                            target_tax_time = pd.Timestamp(year=one_day_before.year, month=one_day_before.month, day=one_day_before.day, hour=15, minute=30)
                            if target_tax_time in target_df['datetime'].values:
