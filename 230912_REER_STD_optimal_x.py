...
for x in range(100, 600):
    ...
    rmse = calculate_rmse(data.loc[data['reer_std'] != 0]['won PV'], data.loc[data['reer_std'] != 0]['reer_std'])
    print(x, rmse)
...
for ii in range(best_x, len(data)):
    data.loc[best_x + kk, 'reer_std'] = (data.loc[ii - best_x:ii - 1, 'won BIS 64'].mean() * data.loc[ii - best_x:ii - 1, 'won PV'].mean())/data.loc[ii, 'won BIS 64']
    kk = kk + 1
